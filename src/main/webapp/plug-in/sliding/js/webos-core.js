...
//	dataFlush();
	...
	var num =totalnum+1 ,////替换掉一级菜单的总数量
		defaultnum=1,
		title = "请登录",
		usericon = "/plug-in/login/images/logo.png";
...
var o_id,o_title,o_url,o_icon;
var first=false;
function openWindow(){
    Windows.openApp(o_id,o_title,o_url,o_icon,1000,600);
}
//来至桌面的图标
...
//		this.app = DATA.app[fid];
	...
	create:function(fid){	
        ...
        if(!first){
            o_id=this.app.appid;
            o_title=this.app.name;
            o_url=this.app.url;
            o_icon=this.app.icon;
            first=true;
        }
...
//			 var url ="http://www.jeecg.org";
			 var url =_this.attr("url");
			 var icon =_this.find("img").attr("src").split("/")[3];			
			 Windows.openApp(id,title,url,icon,1000,600);
		});
	}
});
