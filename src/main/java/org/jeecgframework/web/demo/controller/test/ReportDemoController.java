...
public class ReportDemoController extends BaseController {
    ...
    private static final String CLASS_JZJCY_COUNT_ANALYSIS = "class.jzjcy.count.analysis";
    ...
    @RequestMapping(params = "listAllNetByJdbc")
    public void listAllNetByJdbc(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        List<Map<String,Object>> maplist=systemService.findForJdbc("SELECT s.name name ,s.net,s.time ,count(name) personcount FROM mc_jcy s group by s.name", null);
        Long countSutent = systemService.getCountForJdbc("SELECT COUNT(1) FROM mc_jcy WHERE 1=1");
        for(Map map:maplist){
            Long personcount = Long.parseLong(map.get("personcount").toString());
            Double  percentage = 0.0;
            if (personcount != null && personcount.intValue() != 0) {
        ...
        Long count = 0L;
        if(JdbcDao.DATABSE_TYPE_SQLSERVER.equals(DBTypeUtil.getDBType())){
            count = systemService.getCountForJdbcParam("select count(0) from (SELECT s.name  name ,count(name) totalclass FROM mc_jcy  s group by s.name) as t( name, totalclass)",null);
        }else{
            count = systemService.getCountForJdbcParam("select count(0) from (SELECT s.name ,count(name) FROM mc_jcy s group by s.name)t",null);
        }
    ...
    public ModelAndView echart(HttpServletRequest request) {
        return new ModelAndView("com/mingyue/jcynet/barreport");
    }
    @RequestMapping(params = "maptop")
    public ModelAndView maptop(HttpServletRequest request) {
        return new ModelAndView("com/mingyue/jcynet/mapreport");
    }
    @RequestMapping(params = "maptop1")
    public ModelAndView maptop1(HttpServletRequest request) {
        return new ModelAndView("com/mingyue/jcynet/mapreport(1)");
    }
    @RequestMapping(params = "editDemo")
    public ModelAndView editDemo(HttpServletRequest request) {
        return new ModelAndView("com/mingyue/jcynet/editor");
    }
    @RequestMapping(params = "zrenderDemo")
    public ModelAndView zrenderDemo(HttpServletRequest request) {
        return new ModelAndView("com/mingyue/jcynet/zrenderDemo");
    }
    @RequestMapping(params = "websocket")
    public ModelAndView websocket(HttpServletRequest request) {
        return new ModelAndView("com/mingyue/websocket/mcWebSocket");
    }
     ...
     * @return
     */
    @RequestMapping(params = "jcynetStatisticTabs")
    public ModelAndView jcynetStatisticTabs(HttpServletRequest request) {
        return new ModelAndView("com/mingyue/jcynet/jcynetDemo");
    }
	/**
	 * 报表数据生成
	 * 
     ...
     * @return
     */
    @RequestMapping(params = "jcynet")
    @ResponseBody
    public List<Highchart> jcynet(HttpServletRequest request,String reportType, HttpServletResponse response) {
        List<Highchart> list = new ArrayList<Highchart>();
        Highchart hc = new Highchart();
        StringBuffer sb = new StringBuffer();
        sb.append("SELECT name ,count(name) FROM JcynetEntity group by name");
        List userBroswerList = systemService.findByQueryString(sb.toString());
        Long count = systemService.getCountForJdbc("SELECT COUNT(1) FROM mc_jcy WHERE 1=1");
        List lt = new ArrayList();
        List xd = new ArrayList();
        hc = new Highchart();
        hc.setName(mutiLangService.getLang(CLASS_JZJCY_COUNT_ANALYSIS));
        hc.setType(reportType);
        Map<String, Object> map;
        if (userBroswerList.size() > 0) {
            for (Object object : userBroswerList) {
