...
public class UserController extends BaseController {
    ...
    @RequestMapping(params = "mzyuser")
    public String mzyuser(HttpServletRequest request) {
        // 给部门查询条件中的下拉框准备数据
        List<TSDepart> departList = systemService.getList(TSDepart.class);
        request.setAttribute("departsReplace", RoletoJson.listToReplaceStr(departList, "departname", "id"));
...
//        }
        ...
        return "com/mingyue/mzy/mzyUserList";
    }
    ...
    public AjaxJson savenewpwd(HttpServletRequest request) {
        ...
        if (!pString.equals(user.getPassword())) {
            ...
            try {
                ...
                user.setPhonePw(PasswordUtil.encrypt(user.getMobilePhone(), newpassword, PasswordUtil.getStaticSalt()));
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public AjaxJson savenewpwdforuser(HttpServletRequest req) {
        ...
        if (StringUtil.isNotEmpty(id)) {
            ...
            users.setPhonePw(PasswordUtil.encrypt(users.getMobilePhone(), password, PasswordUtil.getStaticSalt()));
...
//    private boolean isPeopleManager(TSUser tsUser){
//        List<TSRoleUser> tsRoleUserList =systemService.findByProperty(TSRoleUser.class,"TSUser.id",tsUser.getId());
//        for(TSRoleUser tsRoleUser:tsRoleUserList){
//            if(tsRoleUser.getTSRole().getRoleCode().endsWith("peopleManager")){
//                return true;
//            }
//        }
//        return false;
//    }
    @RequestMapping(params = "mzydatagrid")
    public void mzydatagrid(TSUser user, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
        //查询条件组装器
        org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, user);
...
//        roleNameList.add("j_coustom");
//        roleNameList.add("p_coustom");
        TSRole tsRole = systemService.findUniqueByProperty(TSRole.class, "roleCode", "j_coustom");
        roleNameList.add(tsRole.getId());
        ...
        subCq.setProjection(Property.forName("TSUser.id"));
        subCq.in("TSRole.id", roleNameList.toArray());
        subCq.add();
...
//            users.setTSDepart(user.getTSDepart());
            ...
            if (!users.getMobilePhone().endsWith(user.getMobilePhone())) {
                passwordchange = true;
            }
            users.setMobilePhone(user.getMobilePhone());
            ...
            try {
                if (user.getPicPath().indexOf(pa) != -1) {
                    req.setCharacterEncoding("UTF-8");
                    String realPath = req.getSession().getServletContext().getRealPath("/") + "/" + user.getPicPath();// 文件的硬盘真实路径
                    ...
                    users.setUserContent(pcontent);
                    if (ffname.length() > 0 && ext.length() > 0) {
                        users.setExtend(ext);
                        users.setUserClas(ffname);
                    }
                }
                systemService.updateEntitie(users);
                ...
                e.printStackTrace();
                message = "用户: " + users.getUserName() + "更新失败";
            }
            ...
            if (users != null) {
                ...
                users = systemService.findUniqueByProperty(TSUser.class, "mobilePhone", user.getMobilePhone());
                if (users != null) {
                    message = "用户: " + users.getUserName() + "电话号码:" + users.getMobilePhone() + " 已经存在";
                } else {
//				if (user.getTSDepart().equals("")) {
//					user.setTSDepart(null);
//				}
                    user.setPhonePw(PasswordUtil.encrypt(user.getMobilePhone(), password, PasswordUtil.getStaticSalt()));
                    ...
                    try {
                        ...
                        if (user.getPicPath().indexOf(pa) != -1) {
                            ...
                            StreamUtils streamUtils = new StreamUtils();
                            byte[] pcontent = streamUtils.getBytes(fins);
                            user.setUserContent(pcontent);
                            if (ffname.length() > 0 && ext.length() > 0) {
                                user.setExtend(ext);
                                user.setUserClas(ffname);
                            }
                        }
                        ...
                        e.printStackTrace();
                        message = "用户: " + user.getUserName() + "添加失败";
                    }
                }
    ...
    @RequestMapping(params = "mzyAddorupdate")
    public ModelAndView mzyAddorupdate(TSUser user, HttpServletRequest req) {
        List<TSDepart> departList = new ArrayList<TSDepart>();
        String departid = oConvertUtils.getString(req.getParameter("departid"));
        if (!StringUtil.isEmpty(departid)) {
            departList.add((TSDepart) systemService.getEntity(TSDepart.class, departid));
        ...
        return new ModelAndView("com/mingyue/mzy/mzyUser");
    }
    /**
     * 用户的登录后的组织机构选择页面
     *
