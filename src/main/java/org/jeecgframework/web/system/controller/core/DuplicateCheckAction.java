...
public class DuplicateCheckAction extends BaseController {
	...
	public AjaxJson doDuplicateCheck(DuplicateCheckPage duplicateCheckPage, HttpServletRequest request) {
		...
		if(num==null||num==0){
			...
			//该值不可用
            String sql = "SELECT * FROM "+duplicateCheckPage.getTableName()
                    +" WHERE "+duplicateCheckPage.getFieldName() +" =?";
            Map<String,Object> mp=jdbcDao.findOneForJdbc(sql, new Object[]{duplicateCheckPage.getFieldVlaue()});
            List<TSUserOrg> tsUserOrgList=systemService.findByProperty(TSUserOrg.class,"tsUser.id",mp.get("id"));
            String bm=tsUserOrgList.get(0).getTsDepart().getDepartname();
            j.setSuccess(false);
//			j.setMsg("该值不可用，系统中已存在！");
            j.setMsg("该用户已存在！所属店面是 "+bm);
		}
		return j;
	}
}
