...
public class CommonDao extends GenericBaseCommonDao implements ICommonDao, IGenericBaseCommonDao {
     ...
     * 检查用户是否存在
     */
    public TSUser getUserByUserIdAndPhoneExits(TSUser user) {
        String password = PasswordUtil.encrypt(user.getUserName(), user.getPassword(), PasswordUtil.getStaticSalt());
//        String query = "from TSUser u where u.userName = :username and u.password=:passowrd";
        String query = "from TSUser u where u.mobilePhone = :mobilephone and u.phonePw=:passowrd";
        Query queryObject = getSession().createQuery(query);
        queryObject.setParameter("mobilephone", user.getUserName());
        queryObject.setParameter("passowrd", password);
        List<TSUser> users = queryObject.list();
        if (users != null && users.size() > 0) {
            return users.get(0);
    ...
    public void pwdInit(TSUser user, String newPwd) {
        ...
        String query1 = "from TSUser u where u.mobilePhone = :mobilePhone ";
        Query queryObject1 = getSession().createQuery(query1);
        queryObject1.setParameter("mobilePhone", user.getMobilePhone());
        List<TSUser> users1 = queryObject1.list();
        if (null != users1 && users1.size() > 0) {
            user = users1.get(0);
            String pwd = PasswordUtil.encrypt(user.getMobilePhone(), newPwd, PasswordUtil.getStaticSalt());
            user.setPhonePw(pwd);
            save(user);
        }
    }
        ...
        if (uploadFile.getFileKey() != null) {
            ...
            try {
                ...
                if (entityName.equals("TSTemplate")) {
                    ...
                    path = uploadFile.getCusPath() + "/";
                } else if (entityName.equals("McuiEntity")) {
                    realPath = uploadFile.getMultipartRequest().getSession().getServletContext().getRealPath("/") + ResourceUtil.getConfigByName("mcuipath") + "/";
                    path = ResourceUtil.getConfigByName("mcuipath") + "/";
                }
                String fileName = "";
                String swfName = "";
...
//				FileCopyUtils.copy(mf.getBytes(), savefile);
    ...
    public List<TreeGrid> treegrid(List all, TreeGridModel treeGridModel) {
        ...
        for (Object obj : all) {
            ...
            if (StringUtils.isNotEmpty(treeGridModel.getOpenDate())) {
                String openDate = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getOpenDate()));
                tg.setOpendate(openDate);
            }
            if (StringUtils.isNotEmpty(treeGridModel.getDepartCode())) {
                String departCode = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getDepartCode()));
                tg.setDepartCode(departCode);
            }
            if (StringUtils.isNotEmpty(treeGridModel.getCustomCode())) {
                String customCode = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getCustomCode()));
                tg.setCustomCode(customCode);
            }
            if (StringUtils.isNotEmpty(treeGridModel.getTel())) {
                String tel = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getTel()));
                tg.setTel(tel);
            }
            if (StringUtils.isNotEmpty(treeGridModel.getAddress())) {
                String address = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getAddress()));
                tg.setAddress(address);
            }
            if (StringUtils.isNotEmpty(treeGridModel.getMainName())) {
                String mainname = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getMainName()));
                tg.setMainName(mainname);
            }
            if (StringUtils.isNotEmpty(treeGridModel.getIp())) {
                String ip = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getIp()));
                tg.setIp(ip);
            }
            if (StringUtils.isNotEmpty(treeGridModel.getAssetid())) {
                String assetid = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getAssetid()));
                tg.setAssetid(assetid);
            }
            if (StringUtils.isNotEmpty(treeGridModel.getSrc())) {
                String src = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getSrc()));
                tg.setSrc(src);
            }
            if (StringUtils.isNotEmpty(treeGridModel.getIsLink())) {
                String islink = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getIsLink()));
                tg.setIsLink(islink);
            }
...
//            String src = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getSrc()));
            ...
            if (treeGridModel.getIconName() != null) {
                String iconName = TagUtil.fieldNametoValues(treeGridModel.getIconName(), obj).toString();
                if (iconName != null) {
                    tg.setIconName(iconName);
                } else {
                    tg.setIconName("");
                }
            }
//            tg.setSrc(src);
     ...
     * 构建树形数据表
     */
    public List<McuiTreeGrid> mcuiTreegrid(List all, McuiTreeGridModel mcuiTreeGridModel) {
        List<McuiTreeGrid> mcuiTreeGrids = new ArrayList<McuiTreeGrid>();
        for (Object obj : all) {
            ReflectHelper reflectHelper = new ReflectHelper(obj);
            McuiTreeGrid tg = new McuiTreeGrid();

            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getIp())) {
                String ip = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getIp()));
                tg.setIp(ip);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getGuige())) {
                String guige = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getGuige()));
                tg.setGuige(guige);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getNum())) {
                String num = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getNum()));
                tg.setNum(num);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getxField())) {
                String x = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getxField()));
                tg.setX(x);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getyField())) {
                String y = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getyField()));
                tg.setY(y);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getWidthField())) {
                String width = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getWidthField()));
                tg.setWidth(width);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getHeightField())) {
                String height = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getHeightField()));
                tg.setHeight(height);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getFunctionnameField())) {
                String funName = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getFunctionnameField()));
                tg.setFunctionname(funName);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getPrice())) {
                String price = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getPrice()));
                tg.setPrice(price);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getDiscount())) {
                String discount = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getDiscount()));
                tg.setDiscount(discount);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getLevelField())) {
                String level = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getLevelField()));
                tg.setLevel(level);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getType())) {
                String type = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getType()));
                tg.setType(type);
            }
            String id="";
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getIdField())) {
                id = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getIdField()));
                tg.setId(id);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getTextField())) {
                String text = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getTextField()));
                tg.setText(text);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getSrc())) {
                String src = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getSrc()));
                tg.setSrc(src);
            }
            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getDescription())) {
                String description = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getDescription()));
                tg.setDescription(description);
            }

            if (StringUtils.isNotEmpty(mcuiTreeGridModel.getOrder())) {
                String order = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getOrder()));
                tg.setOrder(order);
            }

            if (mcuiTreeGridModel.getIcon() != null) {
                String iconpath = TagUtil.fieldNametoValues(mcuiTreeGridModel.getIcon(), obj).toString();
                if (iconpath != null) {
                    tg.setCode(iconpath);
                } else {
                    tg.setCode("");
            ...
            if (mcuiTreeGridModel.getIconName() != null) {
                String iconName = TagUtil.fieldNametoValues(mcuiTreeGridModel.getIconName(), obj).toString();
                if (iconName != null) {
                    tg.setIconName(iconName);
                } else {
                    tg.setIconName("");
            ...
            if (mcuiTreeGridModel.getIconName_h() != null) {
                String iconName_h = TagUtil.fieldNametoValues(mcuiTreeGridModel.getIconName_h(), obj).toString();
                if (iconName_h != null&&iconName_h.length()>0) {
                    tg.setIconName_h(iconName_h);
                } else {
                    tg.setIconName_h("");
                }
            }

            if (mcuiTreeGridModel.getParentId() != null) {
                Object pid = TagUtil.fieldNametoValues(mcuiTreeGridModel.getParentId(), obj);
                if (pid != null) {
                    tg.setParentId(pid.toString());
                } else {
                    tg.setParentId("");
            ...
            if (mcuiTreeGridModel.getParentText() != null) {
                Object ptext = TagUtil.fieldNametoValues(mcuiTreeGridModel.getTextField(), obj);
                if (ptext != null) {
                    tg.setParentText(ptext.toString());
                } else {
                    tg.setParentText("");
            ...
            List childList = (List) reflectHelper.getMethodValue(mcuiTreeGridModel.getChildList());

            if (childList != null && childList.size() > 0) {
                tg.setState("closed");
            }
            if (mcuiTreeGridModel.getRoleid() != null) {
                String[] opStrings = {};
                List<TSRoleFunction> roleFunctions = findByProperty(TSRoleFunction.class, "TSFunction.id", id);
                ...
                if (roleFunctions.size() > 0) {
                    for (TSRoleFunction tRoleFunction : roleFunctions) {
                        TSRoleFunction roleFunction = tRoleFunction;
                        if (roleFunction.getTSRole().getId().toString().equals(mcuiTreeGridModel.getRoleid())) {
                            String bbString = roleFunction.getOperation();
                            if (bbString != null) {
                                opStrings = bbString.split(",");
                                break;
            ...
            if (mcuiTreeGridModel.getFieldMap() != null) {
                tg.setFieldMap(new HashMap<String, Object>());
                for (Map.Entry<String, Object> entry : mcuiTreeGridModel.getFieldMap().entrySet()) {
                    Object fieldValue = reflectHelper.getMethodValue(entry.getValue().toString());
                    tg.getFieldMap().put(entry.getKey(), fieldValue);
                }
            }
            if (mcuiTreeGridModel.getFunctionType() != null) {
                String functionType = oConvertUtils.getString(reflectHelper.getMethodValue(mcuiTreeGridModel.getFunctionType()));
                tg.setFunctionType(functionType);
            }
            mcuiTreeGrids.add(tg);
        }
