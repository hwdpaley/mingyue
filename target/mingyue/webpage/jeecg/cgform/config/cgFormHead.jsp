...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" beforeSubmit="deleteUnUsedFiled();" layout="table"
             tiptype="1" action="cgFormHeadController.do?save">
    <!-- tiptype="1" -->
    ...
    <table cellpadding="0" cellspacing="1" class="formtable">
        <tr>
            ...
            <td class="value" colspan="3">
                <input class="inputxt" id="tableName" name="tableName" value="${cgFormHeadPage.tableName}"
                       prefixName=""
                       <c:if test="${not empty cgFormHeadPage.tableName}">readonly="readonly"</c:if> datatype="*"
                       validType="cgform_head,table_Name,id" nullmsg=<t:mutiLang langKey="please.input.table.name"/>>
        ...
        <tr>
            ...
            <td class="value" id="jformPkTypeTd"
                ...
                <select id="jformPkType" name="jformPkType" onchange="jformPkTypeChange();">
                    <option value="UUID" <c:if
                            test="${cgFormHeadPage.jformPkType eq 'UUID'}"> selected='selected'</c:if>><t:mutiLang
                            langKey="common.uuid36bit"/></option>
                    <option value="NATIVE" <c:if
                            test="${cgFormHeadPage.jformPkType eq 'NATIVE'}"> selected='selected'</c:if>><t:mutiLang
                            langKey="common.native.auto.increment"/></option>
                    <option value="SEQUENCE" <c:if
                            test="${cgFormHeadPage.jformPkType eq 'SEQUENCE'}"> selected='selected'</c:if>><t:mutiLang
                            langKey="common.sequence"/></option>
                </select>
            </td>
            <td align="right" id="jformPkSequenceN"
                <c:if test="${cgFormHeadPage.jformPkType ne 'SEQUENCE'}">style="display: none;"</c:if>><label
                    class="Validform_label"> <t:mutiLang langKey="sequence.name"/>:</label></td>
            <td class="value" id="jformPkSequenceV"
                <c:if test="${cgFormHeadPage.jformPkType ne 'SEQUENCE'}">style="display: none;"</c:if>><input
                    id="jformPkSequence" name="jformPkSequence" type="text"
                    class="inputxt" value="${cgFormHeadPage.jformPkSequence}"/> <span class="Validform_checktip"></span>
                <label class="Validform_label" style="display: none;"> <t:mutiLang langKey="sequence.name"/>:</label>
            </td>
        </tr>
        <tr>
            <td align="right"><label class="Validform_label"> <t:mutiLang langKey="table.description"/>: </label></td>
            <td class="value"><input class="inputxt" id="content" name="content" value="${cgFormHeadPage.content}"
                                     datatype="s2-100"> <span class="Validform_checktip"></span></td>
            ...
            <td class="value"><select id="jformType" name="jformType" onchange="jformTypeChange();">
                <option value="1" <c:if test="${cgFormHeadPage.jformType eq '1'}"> selected='selected'</c:if>>
                    <t:mutiLang langKey="single.table"/></option>
                <option value="2" <c:if test="${cgFormHeadPage.jformType eq '2'}"> selected="selected"</c:if>>
                    <t:mutiLang langKey="master.table"/></option>
                <option value="3" <c:if test="${cgFormHeadPage.jformType eq '3'}"> selected="selected"</c:if>>
                    <t:mutiLang langKey="slave.table"/></option>
            </select>

                <div id="relation_type_div" style="display: none;"><input type="radio" name="relationType"
                                                                          <c:if test="${cgFormHeadPage.relationType eq '0' || cgFormHeadPage.relationType ==null }">checked="checked"</c:if>
                                                                          value="0"><t:mutiLang
                        langKey="common.one.to.many"/></input> <input type="radio" name="relationType"
                                                                      <c:if test="${cgFormHeadPage.relationType eq '1' }">checked="checked"</c:if>
                                                                      value="1"><t:mutiLang
                        langKey="comon.one.to.one"/></input> &nbsp;<t:mutiLang langKey="sequence.name"/>:<input
                        class="inputxt" style="width: 30px" id="tabOrder" name="tabOrder"
        ...
        <tr>
            ...
            <td class="value"><select id="isCheckbox" name="isCheckbox">
                <option value="N" <c:if test="${cgFormHeadPage.isCheckbox eq 'N'}"> selected="selected"</c:if>>
                    <t:mutiLang langKey="common.no"/></option>
                <option value="Y" <c:if test="${cgFormHeadPage.isCheckbox eq 'Y'}"> selected="selected"</c:if>>
                    <t:mutiLang langKey="common.yes"/></option>
            </select></td>
            ...
            <td class="value"><select id="isPagination" name="isPagination">
                <option value="Y" <c:if test="${cgFormHeadPage.isPagination eq 'Y'}"> selected='selected'</c:if>>
                    <t:mutiLang langKey="common.yes"/></option>
                <option value="N" <c:if test="${cgFormHeadPage.isPagination eq 'N'}"> selected="selected"</c:if>>
                    <t:mutiLang langKey="common.no"/></option>
            </select></td>
        </tr>
        <tr>
            ...
            <td class="value"><select id="isTree" name="isTree">
                <option value="N" <c:if test="${cgFormHeadPage.isTree eq 'N'}"> selected='selected'</c:if>><t:mutiLang
                        langKey="common.no"/></option>
                <option value="Y" <c:if test="${cgFormHeadPage.isTree eq 'Y'}"> selected='selected'</c:if>><t:mutiLang
                        langKey="common.yes"/></option>
            </select></td>
            ...
            <td class="value"><select id="querymode" name="querymode">
                <option value="single" <c:if test="${cgFormHeadPage.querymode eq 'single'}"> selected="selected"</c:if>>
                    <t:mutiLang langKey="single.query"/></option>
                <option value="group" <c:if test="${cgFormHeadPage.querymode eq 'group'}"> selected="selected"</c:if>>
                    <t:mutiLang langKey="combine.query"/></option>
            </select></td>
        </tr>
        <c:if test="${cgFormHeadPage.jformType eq '2'}">
            <tr id="fb_tb">
                ...
                <td class="value" colspan="3"><input class="inputxt" style="width: 440px" disabled="disabled"
                                                     value="${cgFormHeadPage.subTableStr}"></td>
            </tr>
        </c:if>
        <tr class="tree">
            <td align="right"><label class="Validform_label"> 树形表单父id: </label></td>
            <td class="value"><input class="inputxt" id="treeParentIdFieldName" name="treeParentIdFieldName"
                                     value="${cgFormHeadPage.treeParentIdFieldName}" datatype="s2-100"> <span
                    class="Validform_checktip"></span></td>
            <td align="right"><label class="Validform_label"> 树开表单列: </label></td>
            <td class="value"><input class="inputxt" id="treeFieldname" name="treeFieldname"
                                     value="${cgFormHeadPage.treeFieldname}" datatype="s2-100"> <span
                    class="Validform_checktip"></span></td>
        </tr>
        <tr style="display:none;">
            <td align="right"><label class="Validform_label"> idField: </label></td>
            <td class="value"><input class="inputxt" id="treeIdFieldname" name="treeIdFieldname" value="id"
                                     datatype="s2-100"> <span class="Validform_checktip"></span></td>
    ...
    <div id="tabs" class="easyui-tabs" tabPosition="top" fit="false"
        ...
        <div title='<t:mutiLang langKey="database.property"/>' width="auto"
             style="width: auto; margin: 0px; padding: 0px; overflow: hidden;">
            <div style="height: 25px;" class="datagrid-toolbar"><a id="addColumnBtn" class="easyui-linkbutton"
                                                                   data-options="iconCls:'icon-add'"
                                                                   onclick="addColumnBtnClick();" href="#"><t:mutiLang
                    langKey="common.add.to"/></a> <a
                    id="delColumnBtn" class="easyui-linkbutton" data-options="iconCls:'icon-remove'"
                    onclick="delColumnBtnClick();" href="#"><t:mutiLang langKey="common.delete"/></a></div>
