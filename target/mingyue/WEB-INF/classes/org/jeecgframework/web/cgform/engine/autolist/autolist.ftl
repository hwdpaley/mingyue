		...
		<#if x['field_queryMode']=="single">
			<#if x['field_isQuery']=="Y">
				...
				<#if  (x['field_dictlist']?size <= 0)>
					...
					<input type="text" name="${x['field_id']}" style="width: 100px " align="center" <#if x['field_type']=="Date">class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"</#if>  value="${x['field_value']?if_exists?default('')}" />
					<#else>
					<input type="text" name="${x['field_id']}"  style="width: 100px" align="center"
...
<#if  (config_queryList?size >0)>
	...
	<#if config_querymode == "single">
		...
		<div id="${config_id}Listmm" style="width:120px" align="center">
		<#list config_queryList  as x>
			<#if x['field_isQuery']=="Y">
